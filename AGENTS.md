...
11. **Cross-repo (fork) safety.** This repo is a fork (`origin`) of an upstream (`makeplane/plane`). The default workflow targets `AlexanderShang/plane` (this fork) only; the upstream repo is for code that has been reviewed, approved, and is ready for the wider maintainer audience. Every `gh pr` operation must therefore:
    - Pass `--repo AlexanderShang/plane` explicitly. Do not rely on the gh CLI's default-repo inference, which can pick the upstream and silently open a PR there.
    - Use `--base preview --head <branch>` where the head is a feature branch on the same fork. Cross-fork (`head=fork, base=upstream`) PRs are reserved for changes that have been approved for upstream.
    - Be verified after creation: `gh pr view <n> --json url,baseRepositoryOwner,headRepositoryOwner` and confirm both owners are `AlexanderShang`. If `baseRepositoryOwner` is not `AlexanderShang`, immediately close the PR with a comment and re-open against the correct base. Do not assume the PR landed in the right place just because the command exited 0.
    - Treat "this branch historically opened cross-repo PRs" as evidence about the past, not as a rule. PRs #1-#10 were cross-repo; this rule supersedes that default for the unreviewed work in this branch.

If the user later asks to open a cross-repo PR (deliberately, e.g. for upstream review of approved work), confirm explicitly before passing `--repo makeplane/plane`.
...
4. **`@allow_permission` decorator level must match the ViewSet's URL kwargs.** The decorator's default `level="PROJECT"` (apps/api/plane/app/permissions/base.py:19) does `ProjectMember.objects.filter(project_id=kwargs["project_id"])` — if the ViewSet's URL has no `<uuid:project_id>` kwarg, project_id resolves to `None` and every write call 403s, including admins. The same applies symmetrically to `level="WORKSPACE"` on a route that has no `<str:slug>`. Run `python3 tools/check_viewset_decorators.py` (added in commit after the F1 finding in PR #15) before committing any change to a ViewSet under apps/api/plane/app/views/. The script is a pure-AST check (no Django import), runs in <1s, and exits non-zero on a mismatch. It catches the exact class of bug that PR #15 introduced (the @allow_permission level='PROJECT' default) and would have caught it pre-commit.
